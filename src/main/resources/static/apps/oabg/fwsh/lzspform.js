$(function() {
	...
	function bindButtonAction() {
		/** 绑定保存事件* */
		$("a[data-action=fwsh_lzsp_save]").bind("click", function() {
			save();
		});
	}
	...
	function save() {
		if (form.form("validate")) {
			var fwngid=$("#fwsh_lzspform_id").val();
			var fwztCode=$("#fwsh_lzspform_fwzt").val();
			/** 序列化form* */
				...
				if(r){
					...
					$.ajax({
						...
						success : function(data) {
							...
							if (data.success) {
								showMsg(Msg.saveSuc);
								$("#" + modelName + "_lzspFormWin").window("close");
