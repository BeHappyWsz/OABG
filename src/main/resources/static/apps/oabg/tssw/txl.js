$(function(){
	var URL = {
			lxr: ctx + "/oabg/tssw/lxr",
			
			bm: ctx + "/oabg/tssw/bm",
			//局处室
			jcs:ctx+"/oabg/tssw/jcs",
			//直属单位
			zsdw:ctx+"/oabg/tssw/zsdw",
			//辖市区局
			xsqj:ctx+"/oabg/tssw/xsqj"	
	};
	...
	function bindButton(){
		...
		jcsBtn.bind("click",function(){
			...
			names+="局处室"+",";
			form.form('load',{sjrnames:names.substring(0,names.length-1)});
			jcsBtn.hide();
			$.ajax({
				type : "post",
				url : URL.jcs,
