$(function(){
	var URL = {
			gridData : ctx + "/oabg/tssw/bmgrid"
	};
	
	/** 模块名称* */
	var modelName = "tssw_bm";
	/** 主页列表* */
	...
	function bindButtonAction() {
		/** 绑定保存事件* */
		$("a[data-action=tssw_bmxz]").bind("click", function() {
			data();
			queren();
		});
	}
	...
	function queren(){
		...
		if(rows.length > 0){
			...
			$("#tssw_bmlxr").window("close");
			form.form('load',{sjrnames:names,sjrs:sjrid});
		}else{
			showMsg("请选择");
		}
	}
