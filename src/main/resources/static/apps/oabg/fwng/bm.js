$(function(){
	var URL = {
			gridData : ctx + "/oabg/fwng/bmgrid"
	};
	
	/** 模块名称* */
	var modelName = "fwng_bm";
	/** 主页列表* */
	...
	function renderGrid(){
		grid.datagrid({
			...
			columns:[[
			    ...
			    {field:"realname",title:"姓名",align:"left",halign:"center",width:"150px"}
			]],
			onLoadSuccess :function(){
				var data = grid.datagrid("getData");
				var flag=$("#fwng_bm_flag").val();
	...
	function bindButtonAction() {
		/** 绑定保存事件* */
		$("a[data-action='fwng_bm_zs']").bind("click", function() {
			data();
			queren();
		});
		
		$("a[data-action='fwng_bm_cs']").bind("click", function() {
			data1();
			queren1();
		});
	}
