$(function() {
	...
	var URL = {
		/** 保存表单**/
		doSp : ctx + "/oabg/kqgl/ccspOne/doSp"
	};
	/** 模块名称* */
	var modelName = "ccsp";
	/** 表单* */
	...
	function bindButtonAction() {
		/** 绑定保存事件* */
		$("a[data-action=ccsp-sppp]").bind("click", function() {
			doSp()
		});
	}
	...
	/*$("#ccsp_spform_sftg").combobox({
		onChange : function(n,o){
			if(1 == n || "1" == n){
				$("#ccsp_spform_spyjtr").html("审批意见");
				$("#ccsp_spform_spyj").textbox({
					required:false
				});
			}else{
				$("#ccsp_spform_spyjtr").html("审批意见<span style='color:red'>*</span>");
				$("#ccsp_spform_spyj").textbox({
					required:true
				});
			}
