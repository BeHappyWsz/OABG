$(function(){
	var URL = {
			gridData : ctx + "/oabg/gwjd/grid?gridfrom=2",
			formWin : ctx + "/oabg/gwjd/gwjdsh/form",
		...
		grid.datagrid({
			...
			columns:[[
			    ...
			    {field:"cz",title:"操作",align:"center",halign:"center",width:"200",
			    	formatter: function(value,row,index){
						return "<a class='gwjdshGridLookLcjlBtns' data-myid='"+row.id+"' href='#' style='background-color:#36BDEF;color:#FFFFFF;'>查看流程记录</a>";
					}
			    }
			]],
			onLoadSuccess : function(){
				$(".gwjdshGridLookLcjlBtns").linkbutton({
					plain : true,
					onClick : function(){
	...
	function bindGridToorBar() {
		$("a[data-action='GWJDSH_CZSHBTN']").bind("click",function(){
		...
		$("a[data-action='GWJDSH_BGSSHBTN']").bind("click",function(){
			var row = gridSelectedValid(grid);
			if(row){
		...
		$("a[data-action='GWJDSH_CWSHBTN']").bind("click",function(){
			var row = gridSelectedValid(grid);
			if(row){
