$(function(){
	var URL = {
			gridData : ctx + "/oabg/lwypsp/grid",
			formWin : ctx + "/oabg/lwypsp/form",
			getInfo : ctx + "/oabg/lwypsp/getInfo",
		...
		grid.datagrid({
			...
			columns:[[
			    ...
			    {field:"cz",title:"操作",align:"center",halign:"center",width:"200",
			    	formatter: function(value,row,index){
						return "<a class='lwypspGridLookLcjlBtns' data-myid='"+row.id+"' href='#' style='background-color:#36BDEF;color:#FFFFFF;'>查看流程记录</a>";
					}
			    }
			]],
			onLoadSuccess : function(){
				$(".lwypspGridLookLcjlBtns").linkbutton({
					plain : true,
					onClick : function(){
	...
	function bindGridToorBar() {
		$("a[data-action='LWYPSP_SPBTN']").bind("click",function(){
