$(function(){
	var URL = {
			gridData : ctx + "/oabg/lwypsq/grid",
			deleteUrl : ctx + "/oabg/lwypsq/delete",
			formWin : ctx + "/oabg/lwypsq/form",
			getInfo : ctx + "/oabg/lwypsq/getInfo",
			lcjlFormWin : ctx + "/oabg/lwypsq/lcjlForm"
	};
		...
		grid.datagrid({
			...
			columns:[[
			    ...
			    {field:"cz",title:"操作",align:"center",halign:"center",width:"200",
			    	formatter: function(value,row,index){
						return "<a class='lwypsqGridLookLcjlBtns' data-myid='"+row.id+"' href='#' style='background-color:#36BDEF;color:#FFFFFF;'>查看流程记录</a>";
					}
			    }
			]],
			onLoadSuccess : function(){
				$(".lwypsqGridLookLcjlBtns").linkbutton({
					plain : true,
					onClick : function(){
	...
	function bindGridToorBar() {
		$("a[data-action='LWYPSQ_ADDBTN']").bind("click",function(){
			openFormWin("");
		});
		
		$("a[data-action='LWYPSQ_UPDATEBTN']").bind("click",function(){
		...
		$("a[data-action='LWYPSQ_DELETEBTN']").bind("click",function(){
			deleteByIds();
		});
	}
	...
	function openFormWin(id,lczt){
		var win = $("<div id='" + modelName + "_formWin'></div>").window({
			...
			onLoad : function(){
				if(id){
					if(9 != lczt && "9" != lczt){
						$("a[data-action='LWYPSQ_SUBMITBTN']").addClass("hide");
					}
				}
				formLoadData(id);
	        	closeLoadingDiv();
	...
	function formLoadData(id){
		$.ajax({
			...
			success : function(data){
				...
				$("#" + modelName + "_form_lwypId").combogrid("setValue",data.lwypId);
				$("#" + modelName + "_form_lwypId").combo("setText",data.name);
			}
		});
	}
