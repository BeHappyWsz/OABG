$(function() {
	...
	function bindButtonAction() {
		/** 绑定保存事件* */
		$("a[data-action=XXBSSP_DOSPBTN]").bind("click", function() {
			doSp()
		});
	}
	...
	function doSp(){
		if (form.form("validate")) {
			...
			var sfly = "1" == formData.sfly ? "<span style='color:green'>通过</span>" : "<span style='color:red'>不通过</span>"; 
			$.messager.confirm("系统提示","确认审批"+sfly+"？",function(r){
				if(r){
					$.ajax({
						type : "post",
						url : URL.doSp,
						...
						success : function(data) {
							if (data.success) {
								showMsg(Msg.spSuc);
								$("#" + modelName + "_lyFormWin").window("close");
