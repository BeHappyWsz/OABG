...
@Service
public class Ccsp_service extends AbstractService<KqglDao, Kqgl, Long>{
	@Autowired
	...
	@Autowired
	public Ccsp_service(KqglDao dao) {
		super(dao);
	}
	...
	public DataGrid grid(Map<String,Object> map) {
		...
		if(txl!=null){
			if(role.contains("CZ")&&!txl.getGzzw().getCode().equals("4")){
	...
	public Result doSp(String id,String sftg,String suggestion){
		...
		if(RoleCode.contains("BGSFZR")){
			if("1".equals(StringSimple.nullToEmpty(sftg))){
				lczt.setCode("2");
				lcjl.setCnode("审批");
				User user = yhgl_service.getFgld();
				List<String> rolecode = new ArrayList<String>();
		...
		if(RoleCode.contains("FGLD")&&!RoleCode.contains("JZ")){
			if("1".equals(StringSimple.nullToEmpty(sftg))){
				if(!StringKit.isEmpty(t.getTimes())){
					...
					kqcx.setCcaddress(t.getCcaddress());
		...
		if(RoleCode.contains("JZ")&&!RoleCode.contains("FGLD")){
		...
		return wddbService.saveNewWddb(Long.parseLong(id), "30", t.getName()+"的出差申请", name, lczt.getCode());
	}
